...
function RegisterForm({ done }: { done: (user: UserDto) => void }) {
  ...
  return <form onSubmit={submit} className="grid gap-3">
    ...
    {serverError && <p role="alert" className="rounded-lg bg-rose-50 p-2.5 text-[10px] text-rose-600">{serverError}</p>}
...
export function AuthScreen({ done }: { done: (user: UserDto) => void }) {
  ...
  return <main className="min-h-dvh bg-white md:grid md:grid-cols-[1.05fr_.95fr]">
    ...
    <section className="grid min-h-dvh place-items-center bg-[linear-gradient(180deg,#fff,#fafaff)] p-5"><div className="w-full max-w-md rounded-3xl bg-white p-5 md:p-0">
      <div className="mb-6 flex items-center justify-center gap-2 font-bold text-indigo-600 md:hidden"><span className="grid h-8 w-8 place-items-center rounded-xl bg-indigo-600 text-white">↗</span> Fluxo de Compras</div>
      <div className="mb-7 grid grid-cols-2 rounded-xl bg-slate-100 p-1"><button onClick={() => setMode('register')} className={`rounded-lg py-2 text-xs font-bold ${mode === 'register' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-400'}`}>Criar conta</button><button onClick={() => setMode('login')} className={`rounded-lg py-2 text-xs font-bold ${mode === 'login' ? 'bg-white text-indigo-700 shadow-sm' : 'text-slate-400'}`}>Entrar</button></div>
      <h2 className="text-2xl font-black tracking-tight text-slate-950">{mode === 'register' ? 'Comece com dados reais' : 'Que bom ter você de volta'}</h2><p className="mb-6 mt-2 text-xs leading-5 text-slate-400">{mode === 'register' ? 'Sua conta recebe apenas o plano de categorias. As compras serão cadastradas por você.' : 'Entre para continuar sua análise.'}</p>
      {mode === 'register' ? <RegisterForm done={done} /> : <LoginForm done={done} />}
      <p className="mt-5 flex items-center justify-center gap-1.5 text-[9px] text-slate-400"><ShieldCheck size={13} /> Senha protegida e sessão segura.</p>
    </div></section>
  </main>
}
