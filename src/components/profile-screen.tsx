...
type ProfileSection = 'personal' | 'appearance' | 'camera' | 'preferences' | 'accessibility' | 'security'

function AccordionSection({ id, open, onToggle, icon, title, description, children, tone = 'indigo' }: { id: string; open: boolean; onToggle: () => void; icon: ReactNode; title: string; description: string; children: ReactNode; tone?: 'indigo' | 'violet' | 'amber' }) {
  const iconTone = tone === 'violet' ? 'bg-violet-50 text-violet-600' : tone === 'amber' ? 'bg-amber-50 text-amber-700' : 'bg-indigo-50 text-indigo-600'
  return <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
    <button type="button" aria-expanded={open} aria-controls={id} onClick={onToggle} className="flex min-h-20 w-full items-center gap-3 px-4 py-4 text-left">
      <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-2xl ${iconTone}`}>{icon}</span>
      <span className="min-w-0 flex-1"><strong className="block text-base font-black text-slate-950">{title}</strong><small className="mt-0.5 block text-xs leading-5 text-slate-500">{description}</small></span>
      <ChevronDown size={20} className={`shrink-0 text-slate-400 transition-transform ${open ? 'rotate-180' : ''}`} />
    </button>
    {open && <div id={id} className="border-t border-slate-100 px-4 pb-4 pt-4">{children}</div>}
  </section>
}

function SwitchField({ label, description, checked, onChange, icon }: { label: string; description: string; checked: boolean; onChange: (checked: boolean) => void; icon: ReactNode }) {
    ...
    <form onSubmit={save} className="grid gap-4 py-4">
      <section className="overflow-hidden rounded-3xl bg-[linear-gradient(145deg,#312E81,#635BFF)] text-white shadow-[0_14px_34px_rgba(67,56,202,.28)]">
        <button type="button" aria-expanded={openSection === 'personal'} aria-controls="profile-personal" onClick={() => toggleSection('personal')} className="flex min-h-28 w-full items-center gap-4 p-5 text-left">
          ...
          <ChevronDown size={21} className={`shrink-0 text-white/70 transition-transform ${openSection === 'personal' ? 'rotate-180' : ''}`} />
        </button>
        {openSection === 'personal' && <div id="profile-personal" className="mx-3 mb-3 rounded-2xl bg-white p-4 text-slate-900 shadow-inner">
          <p className="mb-4 text-xs leading-5 text-slate-500">Atualize as informações usadas para identificar sua conta.</p>
      ...
      <AccordionSection id="profile-camera" open={openSection === 'camera'} onToggle={() => toggleSection('camera')} icon={<Camera size={20} />} title="Câmera" description="Defina a câmera inicial e teste a permissão.">
        ...
        <p className="mt-3 rounded-2xl bg-emerald-50 p-3 text-xs leading-5 text-emerald-800">A imagem é processada apenas para ler o QR Code e não fica salva na sua conta.</p>
      ...
      <AccordionSection id="profile-preferences" open={openSection === 'preferences'} onToggle={() => toggleSection('preferences')} icon={<Bell size={20} />} title="Notificações e alertas" description="Controle os avisos enviados pelo aplicativo.">
      ...
      <AccordionSection id="profile-accessibility" open={openSection === 'accessibility'} onToggle={() => toggleSection('accessibility')} icon={<Accessibility size={20} />} title="Acessibilidade" description="Ajustes salvos neste dispositivo." tone="violet">
      ...
      {(isDirty || isSubmitting) && <div className="sticky bottom-2 z-20 rounded-3xl border border-indigo-100 bg-white/95 p-2 shadow-[0_12px_32px_rgba(15,23,42,.18)] backdrop-blur"><PrimaryButton disabled={isSubmitting}>{isSubmitting ? 'Salvando…' : 'Salvar alterações'}</PrimaryButton></div>}
    </form>

    <AccordionSection id="profile-security" open={openSection === 'security'} onToggle={() => toggleSection('security')} icon={<KeyRound size={19} />} title="Segurança" description="Altere sua senha confirmando a senha atual." tone="amber">
      <PasswordForm done={() => setOpenSection(null)} />
    </AccordionSection>
