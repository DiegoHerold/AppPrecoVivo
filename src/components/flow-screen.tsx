...
export function FlowScreen({ data, changeMonth, selectCategory, loading }: {
  ...
  return <div className="relative px-4 pb-8">
    ...
    <nav aria-label="Classificação do relatório" className="-mx-4 mb-3 flex gap-1.5 overflow-x-auto border-b border-slate-100 px-4 py-2 scrollbar-none">
    ...
    <div className="grid grid-cols-3 gap-2"><MetricCard label="Desembolso total" value={brl(data.totalSpent, 0)} detail={String(data.purchaseCount) + (data.purchaseCount === 1 ? ' compra' : ' compras')} /><MetricCard label="Média / mês" value={brl(data.history.reduce((sum, item) => sum + item.totalSpent, 0) / 6, 0)} detail="últimos 6 meses" tone="violet" /><MetricCard label="Consumo estimado" value={brl(data.estimatedConsumption, 0)} detail="competência do mês" tone="cyan" /></div>
    <div className="mt-3 flex gap-2 rounded-xl p-3 text-[9px] leading-4" style={{ background: color + '12', color }}><Info size={15} className="shrink-0" /><p>{data.purchaseCount ? 'Somando as compras em ' + title + ' e todos os seus subníveis.' : 'Ainda não há compras em ' + title + ' neste mês. Você pode continuar navegando pelo plano.'}</p></div>

    <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex items-center justify-between"><h2 className="text-[11px] font-bold">{title} — últimos 6 meses</h2><span className="rounded-lg border border-slate-200 px-2 py-1 text-[8px]">Desembolso</span></div><div className="mt-4 flex h-32 items-end justify-between gap-2">{data.history.map((item) => <div key={String(item.year) + '-' + String(item.month)} className="flex h-full flex-1 flex-col items-center justify-end gap-1"><span className="text-[7px] text-slate-400">{item.totalSpent ? brl(item.totalSpent, 0) : ''}</span><span className="w-full max-w-7 rounded-t transition-all" style={{ height: String(Math.max(item.totalSpent ? 8 : 2, item.totalSpent / max * 96)) + 'px', background: item.month === data.month && item.year === data.year ? color : color + '75' }} /><small className="text-[8px] text-slate-400">{item.label}</small></div>)}</div><div className="mt-3 flex gap-4 text-[8px] text-slate-400"><span><i className="mr-1 inline-block h-2 w-2 rounded-sm" style={{ background: color }} />Desembolso</span><span><i className="mr-1 inline-block h-0.5 w-3 bg-cyan-500" />Consumo estimado</span></div></section>
