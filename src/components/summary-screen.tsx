...
export function SummaryScreen({ purchase, onBack, navigate, deleted }: { purchase: PurchaseDto | null; onBack: () => void; navigate: (screen: AppScreen) => void; deleted: () => void | Promise<void> }) {
  ...
  return <div className="px-4 pb-8"><PageHeader title="Compra analisada" subtitle="Tudo salvo no banco" onBack={onBack} />
    ...
    <div className="mt-2 grid gap-2">{Object.entries(groups).map(([behavior, group]) => { const config = BEHAVIORS[behavior as keyof typeof BEHAVIORS]; const tone = { '--tone-color': config.color, '--tone-bg': config.bg } as CSSProperties; return <article key={behavior} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-2.5"><span className="behavior-tone grid h-8 w-8 place-items-center rounded-lg" style={tone}>{config.icon}</span><div className="flex-1"><strong className="block text-[9px]">{config.label}</strong><small className="text-[7px] text-slate-400">{group.count} {group.count === 1 ? 'produto' : 'produtos'}</small></div><b className="behavior-tone rounded-lg px-2 py-1 text-[10px]" style={tone}>{brl(group.total)}</b></article> })}</div>
